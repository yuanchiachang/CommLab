function [min, decoded_data] = conv_dec(binary_data, impulse_response)
    ...
    decoded_data_length = length(binary_data) ./ height;
    ...
    for i = 1: decoded_data_length
        encoded_bit_slice = binary_data((i-1)*height+1:i*height);
        for j = 1: table_height/2
            for k = 0:1
                ...
                hamming_distance = sum(xor(encoded_bit_slice, generated_bit_slice));
                min = viterbi(j,i) + hamming_distance;
